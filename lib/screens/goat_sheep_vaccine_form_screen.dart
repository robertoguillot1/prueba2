...
class GoatSheepVaccineFormScreen extends StatefulWidget {
  final Farm farm;
  final GoatSheep? selectedAnimal;
  final GoatSheepVaccine? vaccineToEdit;

  const GoatSheepVaccineFormScreen({
    super.key,
    required this.farm,
    this.selectedAnimal,
    this.vaccineToEdit,
  ...
  @override
  State<GoatSheepVaccineFormScreen> createState() => _GoatSheepVaccineFormScreenState();
}

class _GoatSheepVaccineFormScreenState extends State<GoatSheepVaccineFormScreen> {
  ...
  void initState() {
    ...
    if (widget.vaccineToEdit != null) {
      ...
      if (widget.selectedAnimal != null) {
        try {
          _selectedAnimal = widget.farm.goatSheep.firstWhere(
            (a) => a.id == widget.selectedAnimal!.id,
          );
        } catch (e) {
  ...
  Future<void> _saveVaccine() async {
    ...
    try {
      ...
      final vaccine = GoatSheepVaccine(
        id: vaccineId,
      ...
      if (widget.vaccineToEdit == null) {
        await farmProvider.addGoatSheepVaccine(vaccine, farmId: widget.farm.id);
      } else {
        await farmProvider.updateGoatSheepVaccine(vaccine, farmId: widget.farm.id);
      }
  ...
  Future<void> _deleteVaccine() async {
    ...
    if (confirm == true) {
      ...
      try {
        final farmProvider = Provider.of<FarmProvider>(context, listen: false);
        await farmProvider.deleteGoatSheepVaccine(widget.vaccineToEdit!.id, farmId: widget.farm.id);
    ...
    return Scaffold(
      ...
      body: Form(
        ...
        child: ListView(
          ...
          children: [
            // Selección de animal
            DropdownButtonFormField<GoatSheep>(
              ...
              items: widget.farm.goatSheep.map((animal) {
                return DropdownMenuItem<GoatSheep>(
                  value: animal,
                  child: Text(animal.name ?? animal.identification ?? 'Sin ID'),
                );
              }).toList(),
            ...
            TextFormField(
              ...
              decoration: const InputDecoration(
                ...
                hintText: 'Ej: Fiebre aftosa, Brucelosis, etc.',
